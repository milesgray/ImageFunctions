...
from ImageFunctions.layers.learnable import Scale
from .methods import *
...
class SinesCosinesLearnable(SineLearnable):
    ...
    def __init__(self, w0: float=1.0, 
                 scale: float=1.0, 
                 learnable: bool=False):
        super().__init__(w0=w0, scale=scale, learnable=False)
        self.scale_sin = Scale(scale) if learnable else nn.Identity()
        self.scale_cos = Scale(scale) if learnable else nn.Identity()
        
    def forward(self, x: Tensor) -> Tensor:
        x = self.w0 * x
        return torch.cat([self.scale_sin(x.sin()), self.scale_cos(x.cos())], dim=1)