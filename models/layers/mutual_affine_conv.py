    ...
    def __init__(self, in_channels, out_channels, kernel_size, 
        ...
        for i in range(self.num_split):
            ...
            setattr(self, f'fc{i}', nn.Sequential(*[
                nn.Conv2d(in_channels=in_split_rest, 
                          out_channels=int(in_split_rest // reduction), 
                          kernel_size=1, 
                          stride=1,
                          padding=0, 
                          bias=True),
                nn.ReLU(inplace=True),
                nn.Conv2d(in_channels=int(in_split_rest // reduction), 
                          out_channels=in_split * 2, 
                          kernel_size=1, 
                          stride=1, 
                          padding=0, 
                          bias=True),
            ]))
            setattr(self, f'conv{i}', nn.Conv2d(in_channels=in_split, 
                                                out_channels=out_split, 
    ...
    def forward(self, x):
        ...
        for i in range(self.num_split):
            scale, translation = torch.split(getattr(self, f'fc{i}')(torch.cat(x[:i] + x[i + 1:], 1)),
                                             (self.in_split[i], self.in_split[i]), 
                                             dim=1)
            transformed = x[i] * torch.sigmoid(scale) + translation
            output = getattr(self, f'conv{i}')(transformed)
