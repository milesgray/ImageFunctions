...
from ...utils import generate_coords, generate_random_resolution_coords
...
class INRs(nn.Module):
    ...
    def generate_input_coords(self, batch_size: int, img_size: int) -> Tensor:
        ...
        """
        if self.training and self.args.hp.get('progressive_transform.enabled'):
            coords = generate_random_resolution_coords(batch_size, img_size, min_scale=self.min_scale)
        elif self.training and self.args.data.get('concat_patches.enabled'):
            normal_coords = generate_coords(batch_size, img_size)
            patch_coords = generate_random_resolution_coords(batch_size, img_size, scale=self.args.data.concat_patches.ratio)
            coords = torch.cat([normal_coords, patch_coords], dim=2)
        else:
            coords = generate_coords(batch_size, img_size)

        if self.args.hp.inr.input_coord_range_lhs == 0:
            pass
        elif self.args.hp.inr.input_coord_range_lhs == -1:
            coords = coords * 2 - 1
        else:
            raise NotImplementedError(f'Unknown `input_coord_range_lhs`: {self.args.hp.inr.input_coord_range_lhs}')
    ...
    def generate_image(self, inrs_weights: Tensor, img_size: int, return_activations: bool=False) -> Tensor:
        ...
        if self.training and self.args.data.get('concat_patches.enabled'):
            images_raw, patches_raw = images_raw[:, :, :img_size ** 2], images_raw[:, :, img_size ** 2:]
            images = images_raw.view(len(inrs_weights), self.args.data.num_img_channels, img_size, img_size) # [batch_size, num_channels, img_size, img_size]
            patches = patches_raw.view(len(inrs_weights), self.args.data.num_img_channels, img_size, img_size) # [batch_size, num_channels, img_size, img_size]
    ...
    def create_transform(self, in_features: int, out_features: int, layer_type: str='linear',
        ...
        layers = [TYPE_TO_INR_CLASS[layer_type](
            ...
            bias_std=bias_std,
            **self.args.hp.inr.get(f'module_kwargs.{layer_type}', {}))]
    ...
    def compute_bias_std(self, in_features: int, is_coord_layer: bool) -> float:
        if is_coord_layer:
            additional_scale = self.args.hp.inr.bias_coords_layer_std_scale
        else:
            additional_scale = self.args.hp.inr.bias_hid_layer_std_scale
...
class FourierINRs(INRs):
    def init_model(self):
        layers = self.create_transform(
            self.args.hp.inr.coord_dim,
            self.args.hp.inr.layer_sizes[0] // 2,
            layer_type=self.args.hp.inr.coords_layer_type,
            is_coord_layer=True)
        layers.append(INRProxy(create_activation('sines_cosines')))

        hid_layers = []

        for i in range(len(self.args.hp.inr.layer_sizes) - 1):
            if self.args.hp.inr.get('skip_coords') and i > 0:
                input_dim = self.args.hp.inr.layer_sizes[i] + self.args.hp.inr.layer_sizes[0]
            else:
                input_dim = self.args.hp.inr.layer_sizes[i]

            curr_transform_layers = self.create_transform(
                input_dim,
                self.args.hp.inr.layer_sizes[i+1],
                layer_type=self.args.hp.inr.hid_layer_type)
            curr_transform_layers.append(INRProxy(create_activation(self.args.hp.inr.activation)))

            if self.args.hp.inr.residual:
                hid_layers.append(INRResidual(INRSequential(*curr_transform_layers)))
            else:
                hid_layers.extend(curr_transform_layers)

        if self.args.hp.inr.get('skip_coords'):
            layers.append(INRInputSkip(*hid_layers))
        else:
            layers.extend(hid_layers)

        layers.extend(self.create_transform(self.args.hp.inr.layer_sizes[-1], self.args.data.num_img_channels, 'linear'))
        layers.append(INRProxy(create_activation(self.args.hp.inr.output_activation)))

        self.model = nn.Sequential(*layers)

    def compute_weight_std(self, in_features: int, is_coord_layer: bool) -> float:
        if is_coord_layer:
            return self.args.hp.inr.fourier_scale
        else:
...
class StaticINRs(nn.Module):
    def __init__(self, args: Namespace):
        ...
        self.args = args
        if self.args.hp.inr.type == "fourier_inr":
            self.inrs = FourierINRs(self.args)
        elif self.args.hp.inr.type == "hier_fourier_inr":
            self.inrs = HierarchicalFourierINRs(self.args)
        else:
            raise NotImplementedError(f'Unknown INR type: {self.args.hp.inr.type}')

        self.external_params = nn.Parameter(torch.randn(
            self.args.hp.num_inrs, self.inrs.num_external_params))
...
class HierarchicalFourierINRs(FourierINRs, INRs):
    ...
    def __init__(self, args: Namespace):
        ...
        print('Resolutions:', self.generate_img_sizes(self.args.data.target_img_size))

        assert not self.args.data.get('concat_patches.enabled')

    def init_model(self):
        blocks = []
        if self.args.hp.inr.res_increase_scheme.enabled:
            res_argss = [self.create_res_args(i) for i in range(self.args.hp.inr.num_blocks)]
        else:
            resolutions = self.generate_img_sizes(self.args.data.target_img_size)
            res_argss = [self.args.hp.inr.resolutions_params[resolutions[i]] for i in range(self.args.hp.inr.num_blocks)]

        print('resolution:', [c.resolution for c in res_argss])
        print('dim:', [c.dim for c in res_argss])
        print('num_learnable_coord_feats:', [c.num_learnable_coord_feats for c in res_argss])
        print('to_rgb:', [c.to_rgb for c in res_argss])
        num_to_rgb_blocks = sum(c.to_rgb for c in res_argss)

        for i, res_args in enumerate(res_argss):
            # 1. Creating coord fourier feat embedders for each resolution
            coord_embedder = INRSequential(
                INRFourierFeats(res_args),
            ...
            if i > 0:
                ...
                connector_layers = [INRResConnector(
                    ...
                    coord_feat_dim,
                    res_args.dim,
                    self.args.hp.inr.upsampling_mode,
                    **self.args.hp.inr.module_kwargs.se_factorized),
                ]
                if self.args.hp.inr.use_pixel_norm:
                    connector_layers.append(INRPixelNorm())
                connector_layers.append(INRProxy(create_activation(self.args.hp.inr.activation, **self.args.hp.inr.activation_kwargs)))
                connector = INRSequential(*connector_layers)
            else:
                connector = INRIdentity()

            transform_layers = []
            for j in range(res_args.n_layers):
                if i == 0 and j == 0:
                    input_size = coord_feat_dim # Since we do not have previous feat dims
                elif self.args.hp.inr.skip_coords:
                    input_size = coord_feat_dim + res_args.dim
                else:
                    input_size = res_args.dim
                ...
                if self.args.hp.inr.use_noise:
                    transform_layers.append(INRNoiseInjection())

                transform_layers.append(INRProxy(create_activation(self.args.hp.inr.activation, **self.args.hp.inr.activation_kwargs)))

            if res_args.to_rgb or i == (self.args.hp.inr.num_blocks - 1):
                to_rgb_weight_std = self.compute_weight_std(res_args.dim, is_coord_layer=False)
                to_rgb_bias_std = self.compute_bias_std(res_args.dim, is_coord_layer=False)

                if self.args.hp.inr.additionaly_scale_to_rgb:
                    to_rgb_weight_std /= np.sqrt(num_to_rgb_blocks)
                    to_rgb_bias_std /= np.sqrt(num_to_rgb_blocks)

                to_rgb = INRToRGB(
                    res_args.dim,
                    self.args.hp.inr.to_rgb_activation,
                    self.args.hp.inr.upsampling_mode,
                    to_rgb_weight_std,
                    to_rgb_bias_std)
            else:
            ...
            if self.args.hp.inr.skip_coords:
    ...
    def create_res_args(self, block_idx: int) -> args:
        increase_conf = self.args.hp.inr.res_increase_scheme
        num_blocks = self.args.hp.inr.num_blocks
        resolutions = self.generate_img_sizes(self.args.data.target_img_size)
    ...
    def generate_image(self, inrs_weights: Tensor, img_size: int, aspect_ratios=None, return_activations: bool=False) -> Tensor:
        ...
        images = images_raw.view(batch_size, self.args.data.num_img_channels, img_size, img_size) # [batch_size, num_channels, img_size, img_size]
    ...
    def apply_weights(self,
        ...
        for i in range(self.args.hp.inr.num_blocks):
        ...
        if self.args.hp.inr.output_activation == 'tanh':
            out = images.tanh()
        elif self.args.hp.inr.output_activation == 'clamp':
            out = images.clamp(-1.0, 1.0)
        elif self.args.hp.inr.output_activation == 'sigmoid':
            out = images.sigmoid() * 2 - 1
        elif self.args.hp.inr.output_activation in ['none', None]:
            out = images
        else:
            raise NotImplementedError(f'Unknown output activation: {self.args.hp.inr.output_activation}')
    ...
    def generate_exp_img_sizes(self, target_img_size: int) -> List[int]:
        ...
        extra_upscale_factor = target_img_size // self.args.data.target_img_size
        img_sizes = []
        curr_img_size = target_img_size

        for i in range(self.args.hp.inr.num_blocks - 1, -1, -1):
            img_sizes.append(curr_img_size)

            if i == self.args.hp.inr.upsample_block_idx:
    ...
    def generate_linear_img_sizes(self, target_img_size: int) -> List[int]:
        min_size = self.args.hp.inr.res_increase_scheme.min_resolution
        max_size = target_img_size
        img_sizes = np.linspace(min_size, max_size, self.args.hp.inr.num_blocks).astype(int)
