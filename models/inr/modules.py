...
class INRFourierFeats(INRModule):
    ...
    def __init__(self, args: Namespace):
        ...
        if self.args.max_num_fixed_coord_feats > 0:
            self.generate_fixed_feats()

        self.num_external_params = self.args.num_learnable_coord_feats * 2

    def get_num_feats(self) -> int:
        result = self.args.num_learnable_coord_feats
    ...
    def forward(self, x: Tensor, params: Tensor) -> Tensor:
        ...
        num_inrs = params.shape[0]
        learnable_basis = self.args.fourier_scale * params.view(num_inrs, self.args.num_learnable_coord_feats, 2)
    ...
    def generate_fixed_feats(self) -> Tensor:
        num_feats_per_direction = np.ceil(np.log2(self.args.resolution)).astype(int) + 1
        bases = [
        ...
        if num_feats_per_direction * len(bases) > self.args.max_num_fixed_coord_feats:
            if not self.args.use_diag_feats:
                bases = bases[:2]

        # Then, removing extra features...
        if num_feats_per_direction * len(bases) > self.args.max_num_fixed_coord_feats:
            num_exceeding_feats = (num_feats_per_direction * len(bases) - self.args.max_num_fixed_coord_feats) // len(bases)

            if self.args.get('select_high_freq_first'):
        ...
        assert self.basis.shape[0] <= self.args.max_num_fixed_coord_feats, \
            f"num_coord_feats > max_num_fixed_coord_feats: {self.basis.shape, self.args.max_num_fixed_coord_feats}"
