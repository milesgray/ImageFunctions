...
class SizeSampler(nn.Module):
    def __init__(self, args: Namespace):
        ...
        self.args = args
        self.stats_path = self.args.hp.generator.size_sampler.stats_path
        self.stats = np.load(self.stats_path, allow_pickle=True)
        self.pos_embedder = nn.Sequential(
            nn.Linear(1, self.args.hp.generator.size_sampler.pos_emb_dim // 2),
            SinesCosines()
        )
    ...
    def sample_labels(self, batch_size: int) -> Tensor:
        return torch.randint(low=0, high=self.args.data.num_classes, size=(batch_size,))
