...
@register("fcn_locnet")
def fcn_locnet(input_nc=3, 
               output_nc=3, 
               nc=32, 
               n_blocks=3, 
               norm_layer="instance", 
               init_spec={"name": "normal", args: {"gain": 0.02}}, 
               **kwargs):
    norm_layer = get_norm_layer(norm_type=norm_layer) if isinstance(norm_layer, str) else norm_layer
    net = FCN(input_nc=input_nc, output_nc=output_nc, nc=nc, n_blocks=n_blocks, norm_layer=norm_layer)
    init_network_weights(net, init_type=init_spec["name"], gain=init_spec["args"]["gain"])
    return net

@register("fcn_3x32_gctx")
def fcn_3x32_gctx(**kwargs):
    norm_layer = get_norm_layer(norm_type="instance")
    net = FCN(input_nc=3, output_nc=3, nc=32, n_blocks=3, norm_layer=norm_layer)
    init_network_weights(net, init_type="normal", gain=0.02)
    return net


@register("fcn_3x64_gctx")
def fcn_3x64_gctx(**kwargs):
    norm_layer = get_norm_layer(norm_type="instance")
    net = FCN(input_nc=3, output_nc=3, nc=64, n_blocks=3, norm_layer=norm_layer)
    init_network_weights(net, init_type="normal", gain=0.02)
    return net


@register("fcn_3x32_gctx_stn")
