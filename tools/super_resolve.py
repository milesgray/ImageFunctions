...
class SuperResolver:
        ...
        try:
            ...
            for p in tqdm(paths):
                img = self.apply(img_path=p,
                         patch_shape=patch_shape,
                         verbose=verbose)
                frames.append(self.prep_image(img))
            torchvision.io.write_video(out_path, torch.stack(frames))
            return True
        except Exception as e:
    ...
    def save_image(self, data, prefix,
        ...
        try:
            ...
            img_path = str(self.path / self.make_filename(prefix))
    ...
    def show_fig(self, data, title="", figsize=(10,8)):
        try:
            ...
            self.log(f"Failed to show figure:\n{e}")
    ...
    def pred(self, img,
        ...
        norms = self.make_img_coeff(data_norm)
        inp = (inp / norms["inp"]["div"]) - norms["inp"]["sub"]
        inp = inp.clamp_(0, 1)
        if torch.cuda.is_available():
            inp = inp.cuda()
        ...
        pred = (pred * norms["gt"]["div"]) + norms["gt"]["sub"]
    ...
    def make_img_coeff(self, data_norm):
        ...
        try:
            ...
            result = self.dict_apply(result,
                                lambda x: self.dict_apply(x,
                                                    lambda y: torch.FloatTensor(y))
            )
            result['inp'] = self.dict_apply(result['inp'],
                                    lambda x: x.view(1, -1, 1, 1))
            result['gt'] = self.dict_apply(result['gt'],
            ...
            self.log(f"Img coeff fail:\n{e}")
            return data_norm.copy()

    def dict_apply(self, source, fn=lambda x: x):
        try:
            result = source.copy()
            for k,v in result.items():
                result[k] = fn(v)
        except Exception as e:
            self.log(f"dict_apply failure:\t{e}")
            return source
        return result