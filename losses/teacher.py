...
@register("teacher_kd_attn")
class KDAttentionLoss(nn.Module):
...
@register("teacher_correlation_congruence")
class CorrelationCongruenceLoss(nn.Module):
...
@register("teacher_factor_transfer")
...
@register("teacher_svd")
class KDSVDLoss(nn.Module):
    """
...
class VIDLoss(nn.Module):
    ...
    def forward(self, s, t):
        ...
        s_H, t_H = s.shape[2], t.shape[2]
        if s_H > t_H:
            s = F.adaptive_avg_pool2d(s, (t_H, t_H))
        elif s_H < t_H:
            t = F.adaptive_avg_pool2d(t, (s_H, s_H))
        else:
            pass
        pred_mean = self.regressor(s)
        pred_var = torch.log(1.0+torch.exp(self.log_scale))+self.eps
        pred_var = pred_var.view(1, -1, 1, 1)
        neg_log_prob = 0.5*(
            (pred_mean-t)**2/pred_var+torch.log(pred_var)
            )
