"""
https://github.com/zhangjinCV/Attention-Guided-Contextual-Feature-Fusion-Network-for-Salient-Object-Detection/blob/main/Code/saliency_toolbox.py
"""
import os
import cv2
import sys
...
def calculate_measures(gt_dir: Union[str,Path], sm_dir: Union[str,Path], measures: List[str], 
                       save: bool=False, 
                       beta: float=np.sqrt(0.3), 
    ...
    for gt_name in tqdm(glob(os.path.join(gt_dir, '*'))):
        ...
        if os.path.exists(sm_name):
            ...
            if 'MAE' in measures:
                #values['TESTMAE'].append({gt_name : mean_square_error(gt, sm)})
            ...
            if 'S-measure' in measures:
                values['S-measure'].append(Eval_Smeasure(gt, sm))
            ...
            if 'Wgt-F' in measures:
                ...
                #values['TESTFM'].append({gt_name : weighted_fmeasure(gt, sm)})
    ...
    if 'Max-F' in measures:
        print(np.array(values['Precision']).shape)
...
def read_and_normalize(gt_path: Union[str,Path], sm_path: Union[str,Path], 
                       gt_threshold: float=0.5):
    """
    ...
    parameters
    ----------
    gt_path : str, pathlib.Path
        The path to a ground truth map
    sm_path : str, pathlib.Path
...
def norm_img(img: np.ndarray):
    return cv2.normalize(img.astype('float'),
                         None,
                         0.0, 1.0,
                         cv2.NORM_MINMAX)
...
# MAE
def mean_square_error(gt: Tensor, sm: Tensor):
...
def e_measure(gt: Tensor, sm: Tensor):
    """
    ...
    if torch.sum(dgt) == 0:  # if the gt is completely black
        enhanced_matrix = 1.0 - dsm  # only calculate the black area of intersection
    elif torch.mean(dgt) == 1:  # if the gt is completely white
    ...
    value = torch.sum(enhanced_matrix) / (height * width - 1 + eps)
    return value


def alignment_term(dgt: Tensor, dsm: Tensor) -> Tensor:
    # compute global mean
    mu_fm = torch.mean(dsm)
...
def enhanced_alignment_term(align_matrix: Tensor) -> Tensor:
    enhanced = ((align_matrix + 1) ** 2) / 4
    return enhanced


def adptive_binary(sm: Tensor) -> Tensor:
    adaptive_threshold = 2 * sm.mean()

    if adaptive_threshold > 1:
        adaptive_threshold = 1

    binary_sm = (sm >= adaptive_threshold).float()
...
def adaptive_fmeasure(gt, sm, beta):
    ...
    value = (1 + beta ** 2) * np.array(prec) * np.array(recall) / ((beta ** 2 * np.array(prec) + np.array(recall)) + eps)
    return value
...
##---------- Pytorch conversion
class SaliencyMeasure(nn.Module):
    def __init__(self, eps:float = 1e-20):
        super().__init__()
        self.eps = eps

    def calculate(self, gt: Tensor, sm: Tensor) -> Tensor:
        diff = gt - sm
        value = diff.abs().mean()
        return value + self.eps

    def forward(self, gt: Tensor, sm: Tensor) -> Tensor:
        return self.calculate(gt, sm)

class E_Measure(SaliencyMeasure):
    """
        This computes the Enhanced-alignment Measure (E-Measure) between the saliency map and the ground truth
        article: https://arxiv.org/abs/1805.10421
        original code [Matlab]: https://github.com/DengPingFan/E-measure
    """
    def calculate(self, gt: Tensor, sm: Tensor) -> Tensor:
        ...
        if torch.sum(dgt) == 0:  # if the gt is completely black
            ...
            # Normal case:
            # 1.compute alignment matrix
            align_matrix = self._alignment_term(dsm, dgt)
            # 2.compute enhanced alignment matrix
    ...
    def _alignment_term(self, dgt: Tensor, dsm: Tensor) -> Tensor:
        # compute global mean
        mu_fm = torch.mean(dsm)
        mu_gt = torch.mean(dgt)
    ...
    def _enhanced_alignment_term(self, align_matrix: Tensor) -> Tensor:
        enhanced = ((align_matrix + 1) ** 2) / 4
        return enhanced


    def _adptive_binary(self, sm: Tensor) -> Tensor:
        adaptive_threshold = 2 * sm.mean()
...
class E_Measure(SaliencyMeasure):
    def calculate(self, gt: Tensor, sm: Tensor) -> Tensor:
        ...
        if gt_mean == 0:  # if the GT is completely black
            ...
            alpha = 0.5
            measure = alpha * self.sim_object(sm, gt) + (1 - alpha) * s_region(sm, gt)
            if measure < 0:
                measure = 0

        return measure

    def _sim_object(self, gt: Tensor, sm: Tensor):
        # compute the similarity of the foreground in the object level

        sm_fg = sm.clone()
        sm_fg[gt == 0] = 0
        o_fg = self.object_score(sm_fg, gt)

        # compute the similarity of the background
        sm_bg = 1.0 - sm.clone()
        sm_bg[gt == 1] = 0
        o_bg = self.object_score(sm_bg, gt == 0)

        u = torch.mean(gt)
        object_value = u * o_fg + (1 - u) * o_bg
        return object_value

    def _object_score(self, gt: Tensor, sm: Tensor) -> float:
        x = sm[gt == 1].mean()
        # compute the standard deviations of the foreground or background in sm
        sigma_x = sm[gt == 1].std()
        score = 2.0 * x / (x ** 2 + 1.0 + sigma_x + eps)
    ...
    def _sim_region(self, gt: Tensor, sm: Tensor) -> float:
        x, y = self._centroid(gt)
        gt_1, gt_2, gt_3, gt_4, w1, w2, w3, w4 = self._divide_gt(gt, x, y)

        sm_1, sm_2, sm_3, sm_4 = self._divide_sm(sm, x, y)

        q1 = self._ssim(sm_1, gt_1)
        q2 = self._ssim(sm_2, gt_2)
        q3 = self._ssim(sm_3, gt_3)
        q4 = self._ssim(sm_4, gt_4)

        region_value = w1 * q1 + w2 * q2 + w3 * q3 + w4 * q4

        return region_value

    def _ssim(self, gt: Tensor, sm: Tensor) -> float:
        gt = gt.float()
        ...
        # Compute the variance of SM,GT
        sigma_x2 = torch.sum(torch.sum((sm - sm_mean) ** 2)) / (num_pixels - 1 + eps)
        sigma_y2 = torch.sum(torch.sum((gt - gt_mean) ** 2)) / (num_pixels - 1 + eps)

        # Compute the covariance
        sigma_xy = torch.sum(torch.sum((sm - sm_mean) * (gt - gt_mean))) / (num_pixels - 1 + eps)
    ...
    def _centroid(self, gt):
        ...
        if torch.sum(gt) == 0:
            x = torch.round(cols / 2)
            y = torch.round(rows / 2)
        else:
            total = torch.sum(gt)
            i = torch.arange(cols).reshape(1, cols) + 1
            j = torch.arange(rows).reshape(rows, 1) + 1

            x = int(torch.round(torch.sum(torch.sum(gt, 0, keepdims=True) * i) / total))
            y = int(torch.round(torch.sum(torch.sum(gt, 1, keepdims=True) * j) / total))
