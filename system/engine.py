...
class TrainingEngine:
    ...
    def _load_checkpoint(self, comp):
        ...
        if 'd_model' in sv_file:
            ...
            comp['d_optimizer'] = optimizers.make(list(comp['model'].parameters()) + 
                                                  list(comp['d_model'].parameters()), 
                                        sv_file['d_optimizer'], 
                                        load_sd=True)
    ...
    def _make_discriminator(self):
        ...
        if 'd_model' in self.config:
            ...
            d_optimizer = optimizers.make(list(self.model.parameters()) + list(d_model.parameters()), self.config['d_optimizer'])
    ...
    def save_models(self, epoch):
        try:
            ...
            if epoch % self.config.get('epoch_save', 10) == 0:
                torch.save(sv_file,
                    os.path.join(self.save_path, 'epoch-{}.pth'.format(epoch)))
