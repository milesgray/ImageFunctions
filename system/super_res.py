...
class SuperResTrainingEngine:
    ...
    def build(self):
        ...
        if self.config.get('resume') is not None:
            comp = self._load_checkpoint(comp)
        else:
            comp = self._build(comp)
        ...
        self.log(f"epoch start: {comp['epoch_start']}")
    ...
    def _build(self, comp):
        ## LIIF compatible models (ITSNR, LIIF+FourierCoord, etc)
        #Create main end to end encoder + projection to RGB color values and logic to output 2d images
        comp["model"] = models.make(self.config['model'])
        if torch.cuda.is_available():
            comp["model"] = comp["model"].cuda()                
        
        # Optimizer for main LIIF model
        comp["optimizer"] = optimizers.make(comp["model"].parameters(), self.config['optimizer'])

        # Starting epoch number for training - should be 0 unless resuming training
        epoch_start = self.config.get('epoch_start', 0)

        # Set learning rate scheduler config or disable 
        # TODO: Add support for more LR schedulers instead of just the one
        ...
        # Make discriminator Model and Optimizer
        if "d_model" in self.config:
            ...
            # Discriminator OPtimizer - applies to both the LIIF model and the Disciminator Model
            comp["d_optimizer"] = optimizers.make(list(comp["model"].parameters()) + list(comp["d_model"].parameters()), self.config['d_optimizer'])
    ...
    def _load_checkpoint(self, comp):
        if torch.cuda.is_available():
        ...
        if 'd_model' in sv_file:
            comp['d_model'] = models.make(sv_file['d_model'], load_sd=True)
            if torch.cuda.is_available():
                comp['d_model'] = comp['d_model'].cuda()
            comp['d_optimizer'] = optimizers.make(list(comp['model'].parameters()) + list(comp['d_model'].parameters()),
                                        sv_file['d_optimizer'],
                                        load_sd=True)
        ...
        comp['epoch_start'] = sv_file['epoch'] + 1
        # Set LR to previous value

        if 'lr_scheduler' in sv_file:
            comp['lr_scheduler'] = sv_file['lr_scheduler']
        else:
            if self.config.get('multi_step_lr') is None:
                comp['lr_scheduler'] = None
            else:
                comp['lr_scheduler'] = MultiStepLR(optimizer, **self.config['multi_step_lr'])
            for _ in range(epoch_start - 1):
                comp['lr_scheduler'].step()
        return comp
